...
def _setup_auth_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("hr_hiring.auth")
    if logger.handlers:
        return logger
...
<head>
  ...
  <title>Přihlášení — Hranipex HR</title>
...
<body>
  <div class="wrap">
    <div class="card">
      <div class="brand"><div class="brand-dot"></div><span class="brand-name">Hranipex &middot; HR Nábor</span></div>
...
def post_login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if settings.auth_mode != "ldap":
        AUTH_LOGGER.warning("LOGIN_SKIPPED user=%s ip=%s reason=ldap_not_configured", username.strip(), ip)
        return RedirectResponse(url=settings.frontend_url, status_code=302)
...
def auth_status(request: Request) -> dict:
    """Returns current session auth status — used by frontend guard."""
    if settings.dev_auth_bypass and settings.env != "prod":
        ...
        return {"authenticated": True, "auth_mode": "dev", "user": user, "test_mode": settings.test_mode}
    user = request.session.get("user")
    return {"authenticated": bool(user), "auth_mode": settings.auth_mode, "user": user, "test_mode": settings.test_mode}
